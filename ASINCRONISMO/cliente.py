...
def conectar_servidor(ip_servidor, puerto):
    client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client.connect((ip_servidor, puerto))
    return client

def ejecutar_comandos(client, log_file=None):
    while True:
        command = input("> ")
        if not command:
            break
...
def main():
    ...
    client = conectar_servidor(args.ip_servidor, args.puerto)
    ejecutar_comandos(client, log_file)
